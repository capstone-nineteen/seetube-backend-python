...
for i in range(4):
    ...
    # input_audio = 'output_audio.mp3'
    # output_video = 'focused_video_'+str(i+1)+'.mp4'

    # command = ['ffmpeg', '-i', input_video, '-i', input_audio, '-c:v', 'copy', '-c:a', 'aac', '-strict', 'experimental', '-map', '0:v:0', '-map', '1:a:0', output_video]
    ...
    # boundingBox_objects[max_index]에 time, x1, y1, x2, y2 들어있음
    ...
    for k in range(count):
        ...
        x = watchingInfo[startTime[i]-2]['gazeInfo']['x']
        y = watchingInfo[endTime[i]-2]['gazeInfo']['y']
    ...
    val = (".", shorts_url, startTime[i], endTime[i], sceneStealer_gazed_percentage, videoId)
    cur.execute(query, val)
